> [!CAUTION]
> ToriaUltra is in no way affiliated with the Polytoria team. For any bug reports, feature suggestions, or concerns, please submit them via the "Feedback" form in the extension preferences.

# ToriaUltra

ToriaUltra is a quality-of-life browser extension for the Polytoria website.


![image](https://img.shields.io/badge/Google_chrome-4285F4?style=for-the-badge&logo=Google-chrome&logoColor=white)
![image](https://img.shields.io/badge/Microsoft_Edge-0078D7?style=for-the-badge&logo=Microsoft-edge&logoColor=white)
![image](https://img.shields.io/badge/Opera-FF1B2D?style=for-the-badge&logo=Opera&logoColor=white)
![image](https://img.shields.io/badge/Brave-FF1B2D?style=for-the-badge&logo=Brave&logoColor=white)
![image](https://img.shields.io/badge/Vivaldi-EF3939?style=for-the-badge&logo=Vivaldi&logoColor=white)
![image](https://img.shields.io/badge/Firefox-000?style=for-the-badge&logo=Firefox&logoColor=white)

## Highlighted features:

- **Pinned Worlds** lets you pin your favorite worlds to the top of your homepage!

- **IRL Brick Price** shows the real life currency value beside bricks across the site.

- **Store "OWNED" Tags** let you see if you own an item in the store at a glance.

- **Collectibles' Hoarders List** offers transparency into people who are hoarding copies of a collectible item.

- **Trade Manager** lets you view, sort, bulk-decline, and counter trades all in one place.

- **Advanced Forum Search** replaces Polytoria's default forum search with one that is faster and gives you more granularity.

- **Legacy Store Layout** reverts the store pages to the old layout from before February 2026

- **Theme Creator** lets you unleash your creativity and customize the Polytoria website to your liking!


## Supported Browsers

ToriaUltra is supported on Chromium-based browsers and Firefox!

### Examples of Browsers that are Chromium-based

- Google Chrome
- Chromium
- Edge
- Opera (& it's variations)
- Brave
- Helium
- Etc..

# How to Download
ToriaUltra isn't out yet, however you can compile it for yourself. This is not recommended as right now because I have not finished stripping out PolyTrack and Kiln APIs.